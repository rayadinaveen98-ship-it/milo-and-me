  ...
  MigrationStrategy get migration => MigrationStrategy(
    onCreate: (m) async {
      await customStatement(
        'CREATE TABLE world (id INTEGER PRIMARY KEY, payload TEXT NOT NULL)',
      );
      await customStatement(
        'CREATE TABLE session (id INTEGER PRIMARY KEY, seconds INTEGER NOT NULL)',
      );
      await customStatement(
...
void main() {
  ...
  test(
    'Actual schema-1 upgrade moves artwork atomically and preserves all progress',
    () async {
      final dir = await Directory.systemTemp.createTemp('milo-v1');
      ...
      final w = World(
        ...
        memories: [
          Memory(
            ...
            topic: 'flower',
            at: DateTime(2026),
            payload: {'strokes': [], 'thumbnail': 'original'},
          ),
        ],
      );
      await old.customStatement('INSERT INTO world VALUES(1,?)', [
        jsonEncode(w.toJson()),
      ]);
      await old.saveDraft('draft', {'strokes': []});
      await old.saveSession(42);
      await old.close();
      final db = AppDatabase(NativeDatabase(file));
      final restored = await db.readWorld();
      expect(restored.nickname, 'Friend');
      expect(restored.petName, 'Sprout');
      expect(restored.memories.single.payload, {'creationId': 'art'});
      expect((await db.readCreation('art'))!['thumbnail'], 'original');
      expect(await db.readDraft('draft'), isNotNull);
      expect(await db.readSession(), 42);
      await db.saveWorld(restored);
      expect((await db.readCreation('art'))!['thumbnail'], 'original');
      await db.reset();
      expect(await db.readCreation('art'), isNull);
      await db.close();
      await dir.delete(recursive: true);
    },
  );
  test(
    'Pack media verifies, installs atomically, reloads lazily offline and uninstalls',
    () async {
      ...
      await repo.load();
      await repo.download(
        url: Uri.parse('https://fixture.test/pack'),
        expectedHash: sha256.convert(data).toString(),
        expectedId: 'media-test',
        expectedVersion: 1,
        progress: (_, _) {},
      );
      ...
      await offline.uninstall('media-test');
      expect(await db.readMedia('media-test', 'voice'), isNull);
      await db.close();
    },
  );
  test(
    'Corrupt downloaded metadata cannot prevent bundled offline startup',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      ...
      expect(repo.list('drawings').length, 24);
      await db.close();
    },
  );
