...
void main() {
  ...
  test('Promotion of a draft to a memory is atomic', () async {
    ...
    expect((await db.readWorld()).nickname, 'Friend');
    await db.close();
  });
  test('Reset removes world, drafts, downloaded content and session', () async {
    final db = AppDatabase(NativeDatabase.memory());
    await db.saveWorld(World(nickname: 'Acorn'));
    await db.saveDraft('flower', {'strokes': []});
    await db.saveSession(300);
    await db.storePack({'id': 'test', 'version': 1});
    await db.reset();
    expect((await db.readWorld()).nickname, '');
    expect(await db.readDraft('flower'), isNull);
    expect(await db.packs(), isEmpty);
    expect(await db.readSession(), 0);
    await db.close();
  });
  test(
    'Opening schema v1 preserves existing state rather than re-creating it',
    () async {
      final dir = await Directory.systemTemp.createTemp('milo-migration-test');
      final file = File('${dir.path}/world.sqlite');
      var db = AppDatabase(NativeDatabase(file));
      await db.saveWorld(World(petName: 'Sprout'));
      await db.close();
      db = AppDatabase(NativeDatabase(file));
