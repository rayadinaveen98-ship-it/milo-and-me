...
void main() {
  ...
  final items = (pack['science'] as List).map((d) => Map<String, dynamic>.from(d)).toList();
  final engine = ScienceEngine();
  test('Seven digital themes and three offline activities all finish with local memories', () {
    ContentEngine.validate(pack);
    expect(items.where((d) => d['safety'] == 'A').map((d) => d['template']).toSet(), ScienceEngine.digital);
    var w = World();
    for (final d in items) {
      for (var i = 0; i < (d['rounds'] as List).length; i++) {
        w = engine.observe(w, d, engine.options(d, i).first, adultPresent: true);
      }
    }
  ...
  test('Safety classification cannot be downgraded or invented', () {
    ...
    expect(() => engine.observe(World(), d, 'I noticed it', adultPresent: false), throwsStateError);
    d['safety'] = 'B';
    expect(() => ScienceEngine.validate(d), throwsFormatException);
    d['safety'] = 'A'; d['template'] = 'chemicals';
    expect(() => ScienceEngine.validate(d), throwsFormatException);
  });
  test('Adult permission is scoped, expires, and revocation beats pending verification', () async {
    ...
    expect(await permit.authorize('leaves', () async { permit.revoke(); return true; }), isFalse);
  });
  test('Offline discovery progress survives actual SQLite reopen', () async {
    ...
    final d = items.first;
    await db.saveWorld(engine.observe(World(), d, engine.options(d, 0).first, adultPresent: false));
    await db.close();
    db = AppDatabase(NativeDatabase(file));
    expect(engine.progress(await db.readWorld(), d['id'])['index'], 1);
    await db.close(); await dir.delete(recursive: true);
  });
  test('Backgrounding revokes science permission independently of parent zone', () async {
    final db = AppDatabase(NativeDatabase.memory());
    final app = AppController(db, ContentRepository(db), World(), audioOverride: SilentAudio());
    ...
    await app.persistSession(); app.dispose();
  });
  testWidgets('Adult-assisted route shows the PIN gate before instructions can advance', (tester) async {
    final db = AppDatabase(NativeDatabase.memory());
    final content = ContentRepository(db);
    await tester.runAsync(content.load);
    final app = AppController(db, content, World(onboarded: true), audioOverride: SilentAudio());
    await tester.pumpWidget(ProviderScope(overrides: [controllerProvider.overrideWith((ref) => app)], child: const MaterialApp(home: ScienceScreen(id: 'leaf-noticing'))));
