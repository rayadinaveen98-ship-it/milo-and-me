...
void main() {
  test(
    ...
    () async {
      final dir = await Directory.systemTemp.createTemp('milo-world');
      ...
      expect(restored.companion.area, 'studio');
      expect(engine.displayedPicture(restored)!.id, 'd0');
      expect(engine.decorations(restored).length, 3);
      expect(engine.displayedPicture(engine.rotatePicture(restored))!.id, 'd1');
  ...
  for (final size in [const Size(430, 932), const Size(1000, 900)]) {
    testWidgets('Six world areas are reachable at ${size.width} pixels', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(home: WorldScreen()),
        ),
      );
