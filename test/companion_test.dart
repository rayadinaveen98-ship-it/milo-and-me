...
void main() {
  ...
  test(
    'Three activity families create distinct callbacks and display slots',
    () {
      var w = World();
      ...
      final lines = <String>[];
      for (var i = 0; i < 60; i++) {
        w = PetEngine().react(w, now: now, sessionMinutes: 0);
        lines.add(w.dialogue);
      }
      for (final kind in ['drawing', 'puzzle', 'story']) {
        expect(lines.any((l) => l.contains('$kind topic')), isTrue);
      }
      expect(w.companion.displaySlots, {
        'picture': 'drawing',
        'puzzle': 'puzzle',
        'story': 'story',
      });
  ...
  test('Long absence greetings preserve energy and affection', () {
    ...
    expect(greeting.energy, 70);
    expect(greeting.affection, 80);
  ...
  test(
    ...
    () async {
      ...
      expect(restored.outfit, 'scarf');
      expect(restored.memories.single.id, 'art');
      await db.close();
