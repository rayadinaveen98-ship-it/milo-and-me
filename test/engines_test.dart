...
void main() {
  ...
  test('Every authored content item passes runtime validation', () => ContentEngine.validate(pack));
  test('Memory is idempotent and unlocks only its activity reward', () {
    final original = World();
    final memory = Memory(id:'drawing:first',kind:'drawing',title:'Flower',topic:'flower',at:DateTime(2026));
  ...
  test('Session ending takes precedence over outfit suggestions', () {
    ...
    expect(PetEngine().suggestion(w,hour:12,sessionMinutes:10),contains('paper'));
  });
  test('Pet remembers a real topic and space outfit affects suggestions', () {
    final w = World(memories:[Memory(id:'one',kind:'drawing',title:'Fish',topic:'fish',at:DateTime(2026))]);
    expect(PetEngine().suggestion(w,hour:12,sessionMinutes:0),contains('fish'));
    w.outfit='astronaut';
    expect(PetEngine().suggestion(w,hour:12,sessionMinutes:0),contains('space'));
  });
  test('Puzzle ordering and length are checked, not just membership', () {
    final puzzle = Map<String,dynamic>.from((pack['puzzles'] as List).firstWhere((p)=>p['id']=='bridge'));
  ...
  test('A branch trapped in a cycle is rejected', () {
    final bad = Map<String,dynamic>.from(jsonDecode(jsonEncode(pack)));
    bad['stories'][0]['scenes']['home']['choices']=[{'label':'Again','next':'home'}];
    expect(()=>ContentEngine.validate(bad),throwsFormatException);
  });
  test('Normalized drawing strokes round-trip including eraser', () {
    final stroke = DrawingStroke(color:0xff344a46,width:.02,erase:true,points:[[.1,.2],[.3,.4]]);
    final roundTrip=DrawingStroke.fromJson(jsonDecode(jsonEncode(stroke.toJson())));
    expect(roundTrip.erase,isTrue);expect(roundTrip.points,stroke.points);
  });
