...
void main() {
  final pack = jsonDecode(File('assets/content/meadow.json').readAsStringSync());
  final story = Map<String, dynamic>.from((pack['stories'] as List).firstWhere((s) => s['id'] == 'missing-moonlight'));
  test('Moonlight routes have distinct resolutions and every scene is interactive', () {
    ...
    expect(File('assets/audio/moonlight-opening.mp3').lengthSync(), greaterThan(1000));
  });
  test('Moonlight ending and interaction survive persistence and completion', () async {
    final db = AppDatabase(NativeDatabase.memory());
    final app = AppController(db, ContentRepository(db), World(onboarded: true), audioOverride: SilentAudio());
    await app.storyPosition('missing-moonlight', 'pond-ending');
    await app.storyInteract('missing-moonlight', 'pond-ending');
    expect((await db.readWorld()).activities['story:missing-moonlight:pond-ending']!['done'], true);
    await app.completeStory(story);
    final saved = await db.readWorld();
    expect(saved.completedStories, contains('missing-moonlight'));
    expect(saved.memories.firstWhere((m) => m.id == 'story:missing-moonlight').payload['ending'], 'pond-ending');
    app.dispose();
  });
}
