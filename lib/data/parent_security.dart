...
class ParentSecurity {
  ...
  Future<void> setPin(String pin) async {
    if (!RegExp(r'^\d{6}$').hasMatch(pin)) throw const FormatException('Choose six digits.');
    final random = Random.secure();
    final salt = base64Encode(List.generate(24, (_) => random.nextInt(256)));
    final hash = await compute(derivePin, [pin, salt]);
    await storage.write(key: 'parent.pin', value: jsonEncode({'salt': salt, 'hash': hash}));
    await storage.delete(key: 'parent.attempts');
  }
  Future<bool> verify(String pin) async {
    final attempts = jsonDecode(await storage.read(key: 'parent.attempts') ?? '{"count":0,"until":0}');
    if (DateTime.now().millisecondsSinceEpoch < (attempts['until'] as int)) throw StateError('Please wait a minute before trying again.');
    ...
    if (difference == 0) { await storage.delete(key: 'parent.attempts'); return true; }
    final count = (attempts['count'] as int) + 1;
    await storage.write(key: 'parent.attempts', value: jsonEncode({'count': count % 5,
      'until': count >= 5 ? DateTime.now().add(const Duration(minutes: 1)).millisecondsSinceEpoch : 0}));
    return false;
  }
}
