      ...
      child: ListView(
        ...
        children: [
          ...
          if (p['display'] != null)
            Padding(
              ...
              child: Text(
                p['display'],
                textAlign: TextAlign.center,
                style: const TextStyle(fontSize: 36, letterSpacing: 5),
              ),
            ),
          if (order) ...[
            Text(
              spatial
                  ? 'Build the bridge from left to right'
                  : 'Tap in order, one at a time',
              textAlign: TextAlign.center,
            ),
              ...
              children: List.generate(
                ...
                (i) => Container(
                  ...
                  decoration: BoxDecoration(
                    color: i < answer.length ? Brand.gold : Colors.white,
                    borderRadius: BorderRadius.circular(spatial ? 8 : 18),
                    border: Border.all(color: Brand.sage),
                  ),
                  child: Text(
                    i < answer.length ? answer[i] : '${i + 1}',
                    textAlign: TextAlign.center,
          ...
          Wrap(
            ...
            spacing: 12,
            runSpacing: 12,
            children: (p['options'] as List)
                .cast<String>()
                .map(
                  (option) => FilledButton.tonal(
                    style: FilledButton.styleFrom(
                      backgroundColor: !order && answer.contains(option)
                          ? Brand.gold
                          : Brand.sky,
                    ...
                    onPressed: busy
                        ...
                        : () => setState(() {
                            feedback = '';
                            if (order) {
                              if (answer.length < count) answer.add(option);
                            } else {
                              answer = [option];
                            }
                          }),
          ...
          Text(
            feedback,
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 18, color: Brand.ink),
          ),
          const SizedBox(height: 16),
          FilledButton(
                ...
                : () async {
                    if (!PuzzleEngine().check(p, answer)) {
                      setState(
                        () => feedback =
                            p['hint'] ??
                            'Let’s look again together. There’s no hurry.',
                      );
                      return;
                    }
                    setState(() => busy = true);
