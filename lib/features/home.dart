    ...
    return PageShell(
      ...
      child: ListView(
        ...
        children: [
                    ...
                    builder: (context, box) {
                      ...
                      return Stack(
                        children: [
                          Positioned.fill(
                            child: CustomPaint(
                              painter: RoomPainter(night: night),
                            ),
                          ),
                          if (drawings.isNotEmpty)
                            Positioned(
                              ...
                              child: GestureDetector(
                                ...
                                child: Semantics(
                                  label: 'Your latest picture on our wall',
                                  button: true,
                                  child: Container(
          ...
          if (w.companion.firsts.isNotEmpty)
            Padding(
              ...
              child: Row(
                ...
                children: [
                  for (final kind in ['drawing', 'puzzle', 'story'])
                    if (w.companion.firsts.contains(kind))
                      Padding(
                        padding: const EdgeInsets.all(8),
                        child: Tooltip(
                          message: 'Our first $kind keepsake',
                          child: Icon(
                            kind == 'drawing'
          ...
          if (w.memories.isEmpty)
            Padding(
              ...
              child: FilledButton.icon(
                onPressed: () => context.go('/drawing/flower'),
                icon: const Icon(Icons.local_florist_rounded),
                label: const Text('Draw our first flower'),
              ),
            ),
          TextButton.icon(
            onPressed: app.suggest,
            icon: const Icon(Icons.lightbulb_outline_rounded),
            label: Text('What shall we do, ${w.petName}?'),
          ),
        ],
      ),
    );
...
class RoomPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    for (var i = 0; i < 12; i++) {
      rect(i / 12, 0, .002, .72, const Color(0x14638A72));
    }
    rect(0, .72, 1, .28, const Color(0xFFE1C8A1));
    rect(0, .71, 1, .02, const Color(0xFFC2A985));
    for (var i = 0; i < 5; i++) {
      rect(0, .76 + i * .06, 1, .002, const Color(0x20A3835A));
    }
    rect(.08, .065, .24, .24, Brand.cream, 45);
    rect(.10, .08, .20, .20, night ? const Color(0xFF526778) : Brand.sky, 40);
    p.color = night ? Brand.cream : Brand.gold;
    c.drawCircle(Offset(s.width * .22, s.height * .14), s.width * .035, p);
    rect(.195, .08, .008, .21, Brand.cream);
    rect(.10, .18, .20, .008, Brand.cream);
    p.color = const Color(0xFFB9CDB6);
    c.drawOval(
      Rect.fromLTWH(
        s.width * .19,
        s.height * .67,
    ...
    for (var i = 0; i < 4; i++) {
      rect(
        ...
        .025,
        .08 + i * .008,
        [Brand.peach, Brand.sage, Brand.lavender, Brand.gold][i],
        3,
      );
    }
    rect(.04, .47, .20, .02, const Color(0xFFB8966C), 5);
    rect(.06, .49, .012, .12, const Color(0xFFB8966C));
    rect(.20, .49, .012, .12, const Color(0xFFB8966C));
  }

  @override
  bool shouldRepaint(covariant RoomPainter old) => old.night != night;
}
