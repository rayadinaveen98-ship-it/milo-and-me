...
class CompanionState {
  ...
  Set<String> firsts;
  CompanionState({this.curiosity = 50, this.interactions = 0, this.cursor = 0,
    this.recentActivity = '', this.recentTopic = '', this.recentReward = '',
    this.stage = 'new-friend', this.careState = 'comfortable', this.area = 'home',
    this.lastInteraction, this.lastSession, List<String>? recentLines,
    Map<String,int>? favourites, Map<String,String>? displaySlots, Set<String>? firsts}) :
    recentLines = recentLines ?? [], favourites = favourites ?? {},
    displaySlots = displaySlots ?? {}, firsts = firsts ?? {};
  Json toJson() => {'schema': 1, 'curiosity': curiosity, 'interactions': interactions,
    'cursor': cursor, 'recentActivity': recentActivity, 'recentTopic': recentTopic,
    'recentReward': recentReward, 'stage': stage, 'careState': careState, 'area': area,
    'lastInteraction': lastInteraction?.toIso8601String(), 'lastSession': lastSession?.toIso8601String(),
    'recentLines': recentLines, 'favourites': favourites, 'displaySlots': displaySlots,
    'firsts': firsts.toList()};
  factory CompanionState.fromJson(Json j) {
    if (j['schema'] != 1) throw const FormatException('Unsupported companion version');
    return CompanionState(curiosity:j['curiosity'], interactions:j['interactions'], cursor:j['cursor'],
      recentActivity:j['recentActivity'], recentTopic:j['recentTopic'], recentReward:j['recentReward'],
      stage:j['stage'], careState:j['careState'], area:j['area'],
      lastInteraction:DateTime.tryParse(j['lastInteraction'] ?? ''),
      lastSession:DateTime.tryParse(j['lastSession'] ?? ''), recentLines:List<String>.from(j['recentLines']),
      favourites:Map<String,int>.from(j['favourites']), displaySlots:Map<String,String>.from(j['displaySlots']),
      firsts:Set<String>.from(j['firsts']));
  }
  void remember(Memory m) {
    recentActivity=m.kind; recentTopic=m.topic; lastInteraction=m.at;
...
class World {
  ...
  World({this.nickname = '', this.petName = 'Milo', this.outfit = 'none',
    ...
    CompanionState? companion, List<Memory>? memories, Set<String>? owned, Set<String>? completedPuzzles,
    Set<String>? completedStories, Map<String, String>? storyPositions}) :
    companion = companion ?? CompanionState(), memories = memories ?? [], owned = owned ?? {'none', 'scarf'},
    completedPuzzles = completedPuzzles ?? {}, completedStories = completedStories ?? {},
    storyPositions = storyPositions ?? {};
  Json toJson() => {'schema': 2, 'companion': companion.toJson(), 'nickname': nickname, 'petName': petName,
  ...
  factory World.fromJson(Json j) {
    if (j['schema'] != 1 && j['schema'] != 2) { throw const FormatException('Unsupported save version'); }
    final companion = j['schema']==2 ? CompanionState.fromJson(Map<String,dynamic>.from(j['companion'])) : CompanionState();
    if(j['schema']==1) { for(final raw in j['memories']) { companion.remember(Memory.fromJson(Map<String,dynamic>.from(raw))); } }
    return World(companion:companion, nickname: j['nickname'], petName: j['petName'], outfit: j['outfit'],
