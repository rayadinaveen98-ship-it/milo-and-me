...
class WorldEngine {
  static const areas=['bedroom','studio','stories','play','care','garden'];
  World visit(World current,String area) {
    if(!areas.contains(area))throw ArgumentError.value(area,'area');
    final w=current.copy();w.companion.area=area;w.mood='walk';
    w.dialogue={
  ...
  Memory? displayedPicture(World w) {
    final drawings=w.memories.where((m)=>m.kind=='drawing'&&m.payload['strokes'] is List);
    if(drawings.isEmpty)return null;
    final selected=w.companion.displaySlots['picture'];
    return drawings.firstWhere((m)=>m.id==selected,orElse:()=>drawings.last);
  }
  World rotatePicture(World current) {
    final w=current.copy();
    final drawings=w.memories.where((m)=>m.kind=='drawing'&&m.payload['strokes'] is List).toList();
    if(drawings.isEmpty)return w;
    final index=drawings.indexWhere((m)=>m.id==w.companion.displaySlots['picture']);
    w.companion.displaySlots['picture']=drawings[(index+1)%drawings.length].id;
    return w;
  }
