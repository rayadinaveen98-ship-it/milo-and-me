...
class PetEngine {
  ...
  World react(
    ...
    if (sessionMinutes >= w.sessionMinutes) {
      ...
      if (w.outfit == 'chef') {
        candidates.add(
          'Our chef hat is ready. Shall we make a pretend picnic?',
        );
      }
      if (w.outfit == 'doctor') {
        candidates.add(
          'Our toy friend would enjoy a cosy story and a blanket.',
        );
      }
      if (w.outfit == 'detective') {
        candidates.add('Shall we look for clues in the moon-button mystery?');
      }
      if (w.outfit == 'builder') {
        candidates.add(
          'Our builder hat is ready for a bridge made of pretend blocks.',
        );
      }
      if (w.outfit == 'beret') {
...
class MemoryEngine {
  World add(World current, Memory memory) {
    ...
    if (memory.kind == 'roleplay' && memory.payload['outfit'] is String) {
      next.owned.add(memory.payload['outfit']);
    }
    return next;
  }
}
